...
router.post(
  ...
  [
    ...
    body("description", "Enter a description with 5 characters")
      .trim()
      .isLength({ min: 5 }),
  ],
  async (req, res) => {
    ...
    try {
      const { title, description, tag, htmlContent, tsContent, cssContent } =
        req.body;
...
router.put("/updateNotes/:id", fetchUser, async (req, res) => {
  const { title, description, tag, htmlContent } = req.body;
  let newNotes = {};

  if (title) newNotes.title = title;
  if (description) newNotes.description = description;
  if (tag) newNotes.tag = tag;
  if (htmlContent) newNotes.htmlContent = htmlContent;
  if (cssContent) newNotes.cssContent = cssContent;
  if (tsContent) newNotes.tsContent = tsContent;
